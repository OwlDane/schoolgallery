    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">Kumpulan foto yang kamu simpan.</p>
            </div>
                    ...
                    @if($g)
                        <a href="{{ route('gallery.detail', $g->id) }}" class="group block bg-white rounded-xl overflow-hidden border border-gray-200 hover:shadow-md transition-all">
                            <div class="relative w-full" style="aspect-ratio: 16/10;">
                                <img src="{{ asset('storage/' . $g->image) }}" alt="{{ $g->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                <div class="absolute top-2 right-2">
                                    <span class="bg-blue-600 text-white text-xs px-2 py-1 rounded-full">Favorit</span>
                                </div>
                            </div>
                            <div class="p-3">
                                <h3 class="text-gray-800 font-semibold line-clamp-2">{{ $g->title }}</h3>
                                <p class="text-xs text-gray-500 mt-1">Disimpan {{ $fav->created_at->diffForHumans() }}</p>
                            </div>
                        </a>
                    @endif
                @endforeach
